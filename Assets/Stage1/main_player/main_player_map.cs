...
public class main_player_map : MonoBehaviour
{
    public Texture[] wink_map = new Texture[44];
    ...
    IEnumerator wink_start()//눈 깜빡임
    {
        ...
        this.wink_error_time = 0.0f;//정상코루틴실행 판단
        this.Gretel_FACE.GetComponent<SkinnedMeshRenderer>().material.mainTexture = this.wink_map[this.wink_index];
        if (wink_index < 42)
        {
            wink_index++;
            StartCoroutine("wink_start");
